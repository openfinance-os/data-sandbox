...
if (!FIXTURES_BUILT) {
  ...
  describe('D-14 generator counterparty-bank breadth — banking personas', () => {
    ...
    for (const [key, fx] of bankingFixtures) {
      ...
      it(`${key} (${segment}) — every BICFI CreditorAgent.Identification across the bundle is a pool BIC`, () => {
        ...
        for (const id of fx.accountIds ?? []) {
          for (const suffix of ['beneficiaries', 'standing-orders', 'scheduled-payments']) {
            ...
            const records =
              env.Data?.Beneficiary ?? env.Data?.StandingOrder ?? env.Data?.ScheduledPayment ?? [];
            for (const rec of records) {
              const ag = rec.CreditorAgent;
              if (!ag) continue;
              expect(ag.SchemeName, `${key} ${suffix} CreditorAgent.SchemeName`).toBe('BICFI');
              expect(
                POOL_BICS.has(ag.Identification),
      ...
      it(`${key} (${segment}) — beneficiary CreditorAgent.Name surfaces real UAE bank names from the pool`, () => {
        ...
        expect(
          names.length,
          `${key} has zero beneficiary records with CreditorAgent.Name`,
        ).toBeGreaterThan(0);
        for (const n of names) {
          expect(
            POOL_BANK_NAMES.has(n),
            `${key} CreditorAgent.Name="${n}" not in counterparty pool`,
          ).toBe(true);
        }
