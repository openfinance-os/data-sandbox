...
if (!FIXTURES_BUILT) {
  ...
  describe('EXP-20 fixture package — @openfinance-os/sandbox-fixtures', () => {
    ...
    it('manifest.json indexes 18 banking + 9 insurance + 8 multi-domain personas × 3 LFIs', () => {
      ...
      expect(Object.keys(m.personas).length).toBe(35);
      expect(Object.keys(m.fixtures).length).toBe(105); // 35 × 3
    ...
    it('ESM loader works — listPersonas + loadFixture + loadSpec', async () => {
      ...
      expect(m.listPersonas({ domain: 'banking' }).length).toBe(26); // 18 + 8 multi
    ...
    it('loadEnrichment returns a complete sidecar for every banking persona', async () => {
      ...
      expect(bankingIds.length).toBe(26); // 18 single-banking + 8 multi-domain
    ...
    it('loadEnrichment payload size matches the bundle transaction count', async () => {
      ...
      const txEnvelope = j.endpoints[`/accounts/${j.accountIds[0]}/transactions`];
    ...
    it('EXP-29 loadJourney returns a coherent bundle for one (persona, lfi, seed)', async () => {
      ...
      expect(accountsEnv?.Data?.Account?.map((a) => a.AccountId).sort()).toEqual(
        [...j.accountIds].sort(),
      );
