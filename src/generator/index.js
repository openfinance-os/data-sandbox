...
export function buildBundle(args) {
  ...
  // Validate EVERY declared domain up front — single- and multi-domain
  // alike. Before this registry existed, buildMultiDomainBundle handled
  // exactly {banking, insurance} by name, so a persona declaring e.g.
  // `domains: [banking, atm]` silently produced a banking-only bundle
  // with no error (APP_IMPROVEMENT_PLAN.md §1 A-6).
  for (const domain of domains) {
    if (!DOMAIN_PIPELINES[domain]) throw new Error(`unknown persona domain: ${domain}`);
  }
  if (domains.length === 1) {
    return DOMAIN_PIPELINES[domains[0]].build(args);
  }
...
function buildMultiDomainBundle({ persona, lfi, seed, pools, now = DEFAULT_NOW, domains }) {
  ...
  // Merge in REGISTRY order (banking → insurance → atm), not persona
  // declaration order: merge order decides which side's scalars win on
  // key collisions, so reordering a YAML `domains:` list must never
  // change generated bytes (EXP-05). buildBundle has already validated
  // every declared domain against the registry.
  for (const domain of Object.keys(DOMAIN_PIPELINES)) {
    if (!domains.includes(domain)) continue;
    bundle = DOMAIN_PIPELINES[domain].mergeInto(bundle, { persona, lfi, seed, pools, now });
  }
 ...
 * Service Initiation) plugs into generation. `build` runs the domain as a
 * standalone bundle; `mergeInto` folds the domain's output into a
 * multi-domain bundle. Any domain missing from this registry is a hard
 * error at buildBundle time, never a silent partial bundle.
 */
