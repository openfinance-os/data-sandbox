...
export const ACCOUNT_SCOPED_PATHS = ENDPOINTS.filter((e) => e.scope === 'account').map(
  (e) => e.path,
);
...
export const INSURANCE_JTBD_PRESETS = Object.freeze({
  ...
  multiDriver: {
    label: 'Multi-driver',
    terms: ['multi_driver_household', 'motor_comprehensive_financed'],
  },
  takaful: { label: 'Takaful', terms: ['sharia_takaful_product'] },
  thirdParty: { label: 'Third-party', terms: ['third_party_liability_only'] },
  mortgageLinked: {
    label: 'Mortgage-linked',
    terms: ['mortgage_linked_home_cover', 'life_mortgage_protection_term'],
  },
  familyCover: {
    label: 'Family cover',
    terms: ['employer_sponsored_health_family', 'renters_furnished_apartment_lease'],
  },
