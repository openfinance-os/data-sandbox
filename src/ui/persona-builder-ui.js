...
export function mountPersonaBuilder({ pools, currentRecipe, onApply }) {
  ...
  function render() {
    ...
    body.appendChild(
      row(
        'Age band',
        selectFor('age_band', ['22-27', '28-38', '39-50', '51-65', '65-72'], recipe.age_band, (v) =>
          update({ age_band: v }),
        ),
      ),
    );
    body.appendChild(
      row(
        'Emirate',
        selectFor('emirate', ['dubai', 'abu_dhabi', 'sharjah', 'other'], recipe.emirate, (v) =>
          update({ emirate: v }),
        ),
      ),
    );
    ...
    if (recipe.segment !== 'Retail') {
      ...
      body.appendChild(
        row(
          'Legal-name pool',
          selectFor('legal_name_pool', orgPoolOptions, recipe.legal_name_pool, (v) =>
            update({ legal_name_pool: v }),
          ),
      ...
      body.appendChild(
        row(
          'Signatory — name pool',
          selectFor('signatory_pool', namePoolOptions, recipe.signatory_pool, (v) =>
            update({ signatory_pool: v }),
          ),
        ),
      );
      body.appendChild(
    ...
    body.appendChild(
      el('h3', {
        ...
        text: `${recipe.segment !== 'Retail' ? '4' : '3'}. Financial profile`,
      }),
    );
    ...
    if (recipe.segment === 'Retail') {
      ...
      body.appendChild(
        row(
          'Employer pool',
          selectFor('employer_pool', employerPoolOptions, recipe.employer_pool, (v) =>
            update({ employer_pool: v }),
          ),
        ),
      );
      body.appendChild(
        row(
          'Income band',
          selectFor('income_band', Object.keys(INCOME_BANDS), recipe.income_band, (v) =>
            update({ income_band: v }),
          ),
      ...
      body.appendChild(
        row(
          ...
          selectFor(
            'cash_flow_intensity',
            ['low', 'med', 'high'],
            recipe.cash_flow_intensity,
            (v) => update({ cash_flow_intensity: v }),
          ),
    ...
    body.appendChild(
      row(
        'Products',
        checkboxGroup('products', ACCOUNT_KINDS, recipe.products, (arr) =>
          update({ products: arr }),
        ),
    ...
    if (recipe.products?.includes('CreditCard')) {
      body.appendChild(
        row(
          ...
          selectFor('card_limit', Object.keys(CARD_LIMITS), recipe.card_limit, (v) =>
            update({ card_limit: v }),
          ),
          Object.entries(CARD_LIMITS)
    ...
    body.appendChild(
      row(
        'Distress level',
        selectFor('distress', Object.keys(DISTRESS_LEVELS), recipe.distress, (v) =>
          update({ distress: v }),
        ),
        Object.entries(DISTRESS_LEVELS)
          .map(([k, [lo, hi]]) => `${k}=${lo}-${hi}/yr NSF`)
          .join(' · '),
      ),
    );
    ...
    const cashBox = checkbox('cash_deposit', recipe.cash_deposit, (b) =>
      update({ cash_deposit: b }),
    );
    ...
    body.appendChild(
      el('h3', {
        class: 'builder-section',
        text: `${recipe.segment !== 'Retail' ? '5' : '4'}. Stress tags (advisory)`,
      }),
    );
    body.appendChild(
      checkboxGroup('stress_tags', STRESS_TAGS, recipe.stress_tags ?? [], (arr) =>
