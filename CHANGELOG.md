...
- **Workstream C — TPP plug-points.**
  ...
  - **Plug-point 4**: **`@openfinance-os/sandbox-mcp`** (new) — Model Context Protocol server wrapping `@openfinance-os/sandbox-fixtures`, so Claude (Desktop, Code, or any MCP client) can drive the sandbox as a dynamic PFM. Banking domain only; 12 curated personas; anonymous (no auth, no API key). Exposes 14 tools (`list_personas`, `set_session`, `get_session`, `get_party`, `get_accounts`, `get_balances`, `get_transactions` with `since`/`until`/`minAmount`/`maxAmount`/`category` filters, `get_standing_orders`, `get_direct_debits`, `get_scheduled_payments`, `get_beneficiaries`, `get_product`, `get_statements`, `load_journey`), 13 resources (`spec://uae-account-information-v2.1`, `persona://<id>` × 12), and 2 prompts (`pick-a-persona`, `monthly-summary`). Two transports under the same package and `--transport` flag: **stdio** (`npx @openfinance-os/sandbox-mcp`, default — Claude Desktop / Claude Code) and **Streamable HTTP** (`--transport http [--port N] [--host H]`, with permissive CORS, `/health` ops endpoint, and per-session isolation so one process can serve many concurrent Claude conversations) — the HTTP path is what the Claude marketplace listing connects to. Every tool response carries the `_watermark` field per EXP-19. PRD decision **D-13** (see §13) accepts this fourth plug-point.
