...
- **Workstream C — TPP plug-points.**
  ...
  - **Plug-point 4**: **`@openfinance-os/sandbox-mcp`** (new) — Model Context Protocol server wrapping `@openfinance-os/sandbox-fixtures`, so Claude (Desktop, Code, or any MCP client) can drive the sandbox as a dynamic PFM. Stdio transport only in v1; banking domain only; 12 curated personas; anonymous (no auth, no API key). Exposes 14 tools (`list_personas`, `set_session`, `get_session`, `get_party`, `get_accounts`, `get_balances`, `get_transactions` with `since`/`until`/`minAmount`/`maxAmount`/`category` filters, `get_standing_orders`, `get_direct_debits`, `get_scheduled_payments`, `get_beneficiaries`, `get_product`, `get_statements`, `load_journey`), 13 resources (`spec://uae-account-information-v2.1`, `persona://<id>` × 12), and 2 prompts (`pick-a-persona`, `monthly-summary`). Every tool response carries the `_watermark` field per EXP-19. The HTTP transport and Claude marketplace listing are gated on PRD decision **D-13** (proposed in this slice — see PRD §13).
