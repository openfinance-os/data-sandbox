...
The five commands that cover ~95% of what you'll do once the hosted MCP endpoint is live, plus the common failure modes worth recognising. Pair this with [`fly.toml`](./fly.toml) and [`.github/workflows/deploy-fly.yml`](./.github/workflows/deploy-fly.yml).
...
(`flyctl tokens create deploy --app of-sandbox-mcp` produces the deploy-only token used by CI as `FLY_API_TOKEN`.)
...
```sh
fly ssh console --app of-sandbox-mcp
```
...
```sh
fly scale show --app of-sandbox-mcp
```

Confirms the VM size and memory match `fly.toml` (default: `shared-cpu-1x` / `256mb`, one machine in `iad`). Resize with:

```sh
fly scale memory 512 --app of-sandbox-mcp                     # bump RAM
fly scale count 2 --region iad --app of-sandbox-mcp           # add a second machine
```

256 MB has fit comfortably in dev — the bundled fixture corpus is ~14 MB JSON, the Node process idle is around 60 MB. Bump only if `fly logs` shows OOM kills.

### 5. `fly secrets list` — what's set in the environment?

```sh
fly secrets list --app of-sandbox-mcp
```
...
|---|---|---|
| `health: critical`, `fly logs` shows the startup banner repeating | Container is crash-looping. Last log line before the banner is usually the real cause | Roll back: `fly releases --app of-sandbox-mcp` then `fly deploy --image registry.fly.io/of-sandbox-mcp:deployment-NNN` to the previous image |
| `health: critical`, no obvious crash, just steady traffic and 5xx's | Memory pressure — `fly logs` will have `OOM killed` lines | `fly scale memory 512 --app of-sandbox-mcp` |
...
| `fly status` shows `version` lower than the latest tag | Last deploy failed silently; the smoke job in `deploy-fly.yml` should have caught it. Check the Actions run | Re-run the workflow, or `fly deploy --remote-only` manually |
...
curl -s https://of-sandbox-mcp.fly.dev/health | jq

# Smoke the deployed endpoint manually
node packages/sandbox-mcp/scripts/smoke.mjs https://of-sandbox-mcp.fly.dev

# Force a redeploy without changing code (e.g. after env-var change)
fly deploy --remote-only --strategy rolling --app of-sandbox-mcp

# Roll back to the previous image
fly releases --app of-sandbox-mcp                             # find the version you want
fly deploy --image registry.fly.io/of-sandbox-mcp:deployment-NNN

# Tail logs for the last hour (e.g. while debugging an incident)
fly logs --app of-sandbox-mcp --since 1h
```
...
- **The smoke job in `deploy-fly.yml` is the green/red signal you trust.** If it failed, the deploy didn't really succeed even if Fly says it did.
