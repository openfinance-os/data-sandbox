...
| **Phase 2.0 — V2 (Insurance preview)** | 2026-Q2 in flight | ◐ In flight | Vendored `uae-insurance-openapi.yaml` pinned by SHA, motor-domain generator (`src/generator/insurance/`), all four motor GET endpoints (`/motor-insurance-policies`, `/motor-insurance-policies/{InsurancePolicyId}`, `/motor-insurance-policies/{InsurancePolicyId}/payment-details`, `/motor-insurance-quotes/{QuoteId}` — every motor-line GET in the v2.1-errata1 spec), three insurance personas (`motor-comprehensive-mid`, `motor-takaful-third-party-expat`, `motor-high-claim-multi-driver`), `status: "preview"` flag in `dist/domains.json`. Detailed scope in `PHASE2_INSURANCE_PLAN.md`. | Phase 2.0 MVP exit (per insurance plan): full v2.1 Insurance endpoint coverage for motor (✅ met — 4 GETs in scope), ≥ 3 insurance personas (✅ met), per-resource Insurance UI replacing the JSON-inspector preview, parity tests with the banking spec-validation suite (✅ insurance suite covers 3 personas × 3 LFI × 4 endpoints = 36 cases). |
