...
| **Phase 2.0 — V2 (Insurance preview)** | 2026-Q2 in flight | ◐ In flight | Vendored `uae-insurance-openapi.yaml` pinned by SHA, motor-domain generator (`src/generator/insurance/`), three motor endpoints, three insurance personas (`motor-comprehensive-mid`, `motor-takaful-third-party-expat`, `motor-high-claim-multi-driver`), `status: "preview"` flag in `dist/domains.json`. Detailed scope in `PHASE2_INSURANCE_PLAN.md`. | Phase 2.0 MVP exit (per insurance plan): full v2.1 Insurance endpoint coverage for motor, ≥ 3 insurance personas (✅ met), per-resource Insurance UI replacing the JSON-inspector preview, parity tests with the banking spec-validation suite (✅ insurance suite covers 3 personas × 3 LFI × 3 endpoints = 27 cases). |
