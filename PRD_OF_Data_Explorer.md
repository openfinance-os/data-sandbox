...
| **Version** | 0.11 (draft — Phase 2.1 GA reconciliation: §3.3 persona library refreshed to 27 (18 banking + 9 insurance) with the six non-motor insurance personas + sme-rak-trading-emirati added; EXP-02 acceptance updated to 9 insurance personas across 7 lines and `status: "ga"`; new Phase 2.1 row in §11. **D-08 flipped from "wire-up deferred" to "wired"**: PostHog landed end-to-end (`src/analytics.js` allowlist shim + lazy SDK loader + `POSTHOG_KEY` secret injection in `tools/stage-site.mjs` + Playwright contract). EXP-21 acceptance tightened to reference the test files. Supersedes v0.10.) |
| **Date** | 10 May 2026 |
...
Distinct from the *user* archetypes above, the sandbox ships a curated library of synthetic UAE customer personas. **As of v2.1 the library is 27 personas: 18 banking personas + 9 insurance personas (status: ga) covering all 7 insurance lines (motor, home, health, life, travel, renters, employment).** Users load these to explore the data:
...
| 13 | **F&B Multi-Outlet Operator** | v2.x | 3-outlet café-and-casual-dining chain in Dubai, weekly aggregator-payout net of commission, daily POS settlement, monthly WPS payroll | Aggregator-vs-POS settlement reconciliation — the canonical F&B accounting pain. Multi-LFI footprint: operating + acquiring + digital-challenger |
| 14 | **E-commerce Marketplace Seller** | v2.x | Free-zone single-shareholder LLC, multi-PSP / multi-marketplace settlement, COD courier remittances, USD-supplier wires | Multi-PSP / marketplace settlement netting against gross orders; the strongest "OF-fed bank feeds replace PSP-portal CSVs" narrative. Multi-LFI footprint: digital-challenger + operating + trade-finance |
| 15 | **Free-Zone SaaS Startup** | v2.x | DIFC-style Series-A SaaS, USD subscription revenue, USD cloud-supplier outflows, AED payroll, two co-founder joint signatories | Dual-currency AED + USD operating reality; FX-active edge under SME segment; investor-round lump-sum receipt pattern. Multi-LFI footprint: operating + trade-finance + digital-challenger |
| 16 | **Construction Sub-Contractor (PDC + Retention)** | v2.x | Sharjah-mainland sub-contractor, milestone-billed certified-payment receivables, 5–10% retention, post-dated-cheque collection, ~80-labourer monthly WPS | PDC handling, retention release matching, large-batch WPS payroll — none exercised by other personas. Multi-LFI footprint: operating + Islamic deposit |
| 17 | **Healthcare Clinic — Insurance Receivables** | v2.x | DHA-licensed mainland clinic, 30–60-day insurer-payor reimbursements net of co-pay + e-claim platform fee, mixed retail-card and B2B-credit flows | Insurance-receivable aging, payor-tagged inflow stream; bridges banking-domain personas with the insurance-domain motor personas without crossing into Open Insurance scope. Multi-LFI footprint: operating + Islamic deposit + digital-challenger |
| 18 | **SME RAK Trading — Emirati** | v2.x | RAK-licensed Emirati trading SME, AED-only counterparties, monthly Islamic finance instalment, mainland trade-finance footprint | Closes the Northern-Emirates / Emirati-SME gap; exercises Islamic-deposit + trade-finance multi-LFI footprint without DIFC / mainland-Dubai bias |
| **I-1** | **Motor Comprehensive — Mid** *(insurance ga)* | v2.0 | Mid-tier motor comprehensive policy, single vehicle, one named driver, no prior claims | First Phase-2 Open Insurance persona; exercises motor policy / vehicle / claims envelopes against the vendored UAE Insurance OpenAPI |
| **I-2** | **Motor Takaful Third-Party — Expat** *(insurance ga)* | v2.0 | Sharia-compliant takaful TPL policy on an older sedan, expat policyholder, Direct channel, no finance, no additional drivers | Exercises `takaful=true`, `TypeOfPolicy=ThirdPartyLiability`, and the no-finance branch of the motor generator that the comprehensive baseline doesn't reach |
| **I-3** | **Motor High-Claim Multi-Driver** *(insurance ga)* | v2.0 | Comprehensive policy on a financed family SUV, three additional named drivers, two claims in trailing 12 months (one approved), Aggregator channel | First persona with `claims_in_period > 0`, multi-driver array of length 3, and Aggregator purchase channel; tests `NoClaimsDiscountAvailable=false` and the populated-`Claims.Summary` shape |
| **I-4** | **Home Mortgage — Villa** *(insurance ga)* | v2.1 | Villa buildings + contents cover bundled with mortgage, single property, no prior home claims | First non-motor insurance persona; exercises home policy / property / contents envelopes and the buildings-vs-contents split |
| **I-5** | **Health Family Comprehensive** *(insurance ga)* | v2.1 | Family group health policy with dependants, comprehensive in-patient + out-patient cover, no prior claims | First health-line persona; exercises dependant array, network tier, and benefit-limit shapes |
| **I-6** | **Life Mortgage Protection** *(insurance ga)* | v2.1 | Decreasing-term life cover assigned to a mortgage, single life assured | First life-line persona; exercises life policy / sum-assured / assignment-to-mortgagee shapes |
| **I-7** | **Travel Annual Multi-Trip — Expat** *(insurance ga)* | v2.1 | Annual multi-trip worldwide travel policy, expat policyholder, no prior claims | First travel-line persona; exercises trip-frequency, region-of-cover, and short-policy-period shapes |
| **I-8** | **Renters Apartment Tenant** *(insurance ga)* | v2.1 | Apartment-tenant renters cover (contents + personal liability) for an expat tenant | First renters-line persona; exercises renters policy / contents-only cover / liability-limit shapes |
| **I-9** | **Employment ILoE — Private Sector** *(insurance ga)* | v2.1 | Mandatory Involuntary Loss of Employment (ILoE) cover for a private-sector salaried expat | First employment-line persona; exercises ILoE category, salary-band, and waiting-period shapes specific to the UAE mandatory scheme |
...
| **EXP-02** | The sandbox SHALL ship a **persona library** of at least 10 named UAE customer archetypes at Phase 1 launch (per §3.3) and at least 12 by Phase 1.5. Personas are versioned and curated against publicly observable UAE-market patterns. Persona definitions contain no information sourced from any specific institution. The Phase 2.0 insurance preview shipped three motor personas; **Phase 2.1 GA expands the insurance library to nine personas across seven lines** (motor, home, health, life, travel, renters, employment) so every line exercises at least one persona, and the domain status flips from `preview` to `ga` in `dist/domains.json`. | At Phase 1 launch ≥ 10 banking personas published; at Phase 1.5 ≥ 12 banking personas (the v1.5 trio recorded as deferred per §3.3); at v2.0 ≥ 3 Open Insurance personas (initially `status: "preview"`); **at v2.1 ≥ 9 Open Insurance personas covering all seven in-scope lines, `status: "ga"` in `dist/domains.json`**. Each has a YAML manifest in `/personas/` with a `stress_coverage` field per EXP-25. Each manifest passes a "no-institution-leak" lint that flags any reference to a specific bank or LFI. | v1 / v1.5 / v2.0 / v2.1 |
...
| ID | Requirement | Acceptance | Phase |
|---|---|---|---|
| **EXP-21** | **Anonymous analytics surface.** PostHog (per D-08), event set restricted to: persona load, LFI switch, field click, endpoint navigation, raw-JSON toggle, export, share. No PII, no fingerprinting, no cross-site identifiers, no user-input strings captured. The PostHog config (project key, autocapture flags, identity/session settings) is treated as part of the deployed bundle and reviewed at every release. | The allowlist lives in `src/analytics.js` (`ALLOWED_EVENTS` + `ALLOWED_PROP_KEYS`). `tests/analytics-allowlist.test.mjs` greps every `track()` call site in `src/`, fails the build on a non-allowlisted event name, fails on a non-allowlisted property key, fails on any PII-shaped key, and fails if an allowlisted event has no call site (drift detector). `tests/e2e/analytics.spec.mjs` stubs PostHog via Playwright route-fulfill, asserts the strict SDK-init options, asserts the actual capture() payload after a real user gesture, and asserts EXP-22's no-cookie / no-localStorage posture. Per-release audit reduces to a CI green check. | v0.11 |
...
- Institution-specific operational detail, internal categorisation rules, or proprietary populate-rate observations — never (NG5, refined by D-14: real UAE bank names are allowed only as counterparty-pool entries and as `plausible_lfi_candidates` — never bound to a populate-rate or to the bundle-emitting LFI).
...
| **Phase 2.0 — V2 (Insurance preview)** | 2026-Q2 | ✅ Shipped | Vendored `uae-insurance-openapi.yaml` pinned by SHA, motor-domain generator (`src/generator/insurance/`), all four motor GET endpoints (`/motor-insurance-policies`, `/motor-insurance-policies/{InsurancePolicyId}`, `/motor-insurance-policies/{InsurancePolicyId}/payment-details`, `/motor-insurance-quotes/{QuoteId}` — every motor-line GET in the v2.1-errata1 spec), three insurance personas (`motor-comprehensive-mid`, `motor-takaful-third-party-expat`, `motor-high-claim-multi-driver`), per-resource Insurance UI (navigator + structured `<dl>` field-tree with parsed-spec status badges, replacing the JSON-inspector preview), `@openfinance-os/sandbox-mcp` insurance support (4 motor tools, domain-aware session, `spec://uae-insurance-v2.1` resource), `status: "preview"` flag in `dist/domains.json`. Detailed scope in `PHASE2_INSURANCE_PLAN.md`. | Phase 2.0 MVP exit (per insurance plan): full v2.1 Insurance endpoint coverage for motor (✅ met — 4 GETs in scope), ≥ 3 insurance personas (✅ met), per-resource Insurance UI replacing the JSON-inspector preview (✅ met), parity tests with the banking spec-validation suite (✅ insurance suite covers 3 personas × 3 LFI × 4 endpoints = 36 cases). |
| **Phase 2.1 — V2.1 (Insurance GA — full line coverage)** | 2026-Q2 | ✅ Shipped | Full read-only GET coverage for every insurance line in the v2.1-errata1 spec: motor + home + health + life + travel + renters + employment + Insurance Consents (30 endpoints across 7 lines + the consents pair). Six new insurance personas land alongside the three motor personas, one per non-motor line: `home-mortgage-villa`, `health-family-comprehensive`, `life-mortgage-protection`, `travel-annual-multitrip-expat`, `renters-apartment-tenant`, `employment-iloe-private`. Domain status flips from `preview` → `ga` in `dist/domains.json` (and `tools/domains.config.mjs:44`). One new SME persona (`sme-rak-trading-emirati`) brings banking to 18. Per-line generator modules under `src/generator/insurance/<line>.js` with a registry-driven dispatcher in `src/generator/insurance/index.js`. | All 30 in-scope insurance GETs resolve for every applicable persona × LFI; AJV strict validation passes (`tests/spec-validation.insurance.test.mjs` + `tests/rendered-fixture-spec-validation.test.mjs`); MCP server surfaces all 27 personas with optional `domain` filter; npm + PyPI packages ship the expanded fixture corpus (2034 files); persona library at 27 (18 banking + 9 insurance). |
| **Phase 2.x — V2 (open scope)** | 2026-Q3+ | Planned | **Contributing guide + SME review process published first**, then community persona contributions (PR-style) opened. Open Wealth domain extension. **Restoration of the deferred v1.5 trio** (Domestic Worker, PEP-flagged, Returning Expat) tracked here. Replay mode (record real Al Tareq sandbox calls and replay deterministically). Arabic / RTL i18n per amended D-10. **PostHog wire-up** per amended D-08 (project key + autocapture config) lands as a v2.x patch and flips EXP-21 acceptance from "trivially true" (zero events emitted) to actively-asserted-against-the-allowlist. | Contributing guide live + ≥ 3 community-contributed personas accepted under it. |
...
| **D-08** *(was OQ-EXP-08; amended v0.10, v0.11)* | **Analytics: PostHog** (the same surface OF-OS Commons already uses). **Wired in v0.11** — `src/analytics.js` enforces an event + property-key allowlist; the SDK is lazy-loaded from `unpkg.com/posthog-js@1` on first allowlisted gesture so the cold-load path stays clean (EXP-24 budget). Project key arrives via `secrets.POSTHOG_KEY` injected by `tools/stage-site.mjs` into `<script>window.__POSTHOG_KEY__='...'</script>`; missing-key state short-circuits to a no-op so a fork/local build never violates EXP-21. SDK init is strict-by-construction: `autocapture`/`capture_pageview`/`capture_pageleave` false, `disable_session_recording`/`disable_surveys` true, `persistence: 'memory'`, fresh `crypto.randomUUID()` distinctID per reload, `$ip`/`$current_url`/`$pathname`/`$referrer`/`$user_agent` etc. blacklisted, `sanitize_properties` routes through the same allowlist as the shim. Project hosted at `us.i.posthog.com`. | Anonymous events only — no PII, no fingerprinting. Captured: persona load, LFI switch, field click, endpoint navigation, raw-JSON toggle, export, share. The lazy-load + memory-persistence design keeps EXP-22 (no-PII / no-cookies) intact: a visitor who never interacts emits zero requests, and a visitor who does interact carries an opaque per-reload session token and nothing else. |
...
| **D-13** *(new; amended Phase 2.0)* | **Conversational plug-point via MCP server is accepted as an additive, read-only wrapper over `@openfinance-os/sandbox-fixtures`.** Shipped in two transports under the same package (`@openfinance-os/sandbox-mcp`): (1) **stdio** for `npx` / Claude Desktop / Claude Code; (2) **Streamable HTTP** (anonymous, CORS-open) for the Claude marketplace listing — canonical endpoint `mcp.openfinance-os.org/mcp` (live during the OF-OS Commons cutover from `data-sandbox.fly.dev/mcp`, the current Fly-hosted deployment). The HTTP path is the only part of the sandbox surface that runs a backend; this is acceptable because the corpus is fully synthetic — there is nothing real to protect, mirroring the §6.4 anonymous posture. Determinism (EXP-05), spec-pin (EXP-10), watermark (EXP-19), no-PII (EXP-07), no-institution-leak (NG5), and no-mandatory-redaction (§8.3) invariants apply unchanged: every tool response carries `_watermark` + `_specSha`, and per-MCP-session state is isolated server-side so concurrent users cannot leak personas across each other. **Phase 2.0 amendment**: the MCP server is now domain-aware — `list_personas` surfaces all 15 personas (12 banking + 3 insurance) with an optional domain filter, and four new motor tools (`get_motor_policies`, `get_motor_policy`, `get_motor_payment_details`, `get_motor_quote`) cover the v2.1-errata1 motor GET surface. Tool count: 20. Banking and insurance tools refuse cross-domain calls with a "switch personas" error so the LLM gets a clear routing signal. | Becomes plug-point 4 alongside the four existing TPP plug-points. Hosting follows OF-OS Commons identity per NG6 (no separate branding). The HTTP endpoint is anonymous: no auth, no API key, no OAuth — install friction is one click. Re-opening this decision would require a security argument that the synthetic-data + watermark guarantees are insufficient. |
| **D-14** *(new — Phase 2.x SME expansion)* | **NG5 refined: real UAE bank names are allowed at named-counterparty sites where no operational claim is bound; forbidden anywhere a populate-rate, product mix, categorisation rule, or other operational claim binds to a name.** **Allowed** sites: (a) the dedicated `synthetic-identity-pool/counterparty-banks/*.yaml` pools used to populate `Beneficiaries`, `StandingOrders`, `Transaction.CounterpartyAccount`, etc. — banks named here are third-party counterparties the persona transacts with, not sources of operational data; (b) the optional `multi_lfi_footprint.{primary,secondary,tertiary}.plausible_lfi_candidates` array in persona manifests — a candidate set with no populate-rate binding. **Forbidden** sites (unchanged from NG5): the `lfi_profile` field, anywhere bound to populate-rate guidance, anywhere identifying the bundle-emitting LFI, and any UI label that implies operational attribution. The `Rich`/`Median`/`Sparse` populate-rate labels remain anonymous. | Resolves the tension between UAE-realism (SMEs are visibly multi-banked across Tier-1 / Tier-2 / Islamic / digital-challenger banks) and NG5 (no operational attribution). The cleanest articulation: NG5 is about *operational attribution*, not *naming*. Naming a counterparty bank says nothing about that bank's data quality; binding a populate-rate to a bank name does. The `lint-no-institution-leak` lint is rewritten in the same slice to encode the allowed/forbidden split exactly. |
...
- **Appendix F — Stress-coverage controlled vocabulary** — manifests' `stress_coverage` arrays draw from this enumeration so EXP-25's CI uniqueness check has stable terms. **v1 set**: `multi_party_accounts`, `joint_custodian`, `power_of_attorney`, `fx_currency_exchange`, `multi_currency_accounts`, `pep_kyc_claims`, `verified_claims_block`, `low_volume_inference`, `cash_dominant_flows`, `high_dbr`, `nsf_distress`, `thin_file_short_tenure`, `tenure_rich_uae_thin`, `remittance_outflow`, `gig_irregular_inflow`, `salary_payroll_flag`, `mortgage_long_dated`, `credit_line_block`, `sharia_compliant_product`, `salary_assigned_lending`. **v2.x SME-expansion additions** (per the 5 new SME personas — F&B multi-outlet, e-commerce marketplace seller, free-zone SaaS, construction sub-contractor, healthcare clinic): `aggregator_settlement_reconciliation`, `multi_psp_settlement_netting`, `usd_aed_sme_dual_currency_ops`, `pdc_retention_milestone_billing`, `insurer_payor_receivables_aging`. The vocabulary is extended (never silently broken) when a new persona introduces a new stress area; additions land in patch revisions.
...
| Author + OF-OS Commons maintainer | Michael Hartmann | v0.11 — Phase 2.1 GA reconciliation. §3.3 persona library refreshed to 27 personas (18 banking + 9 insurance) with the six non-motor insurance personas (`home-mortgage-villa`, `health-family-comprehensive`, `life-mortgage-protection`, `travel-annual-multitrip-expat`, `renters-apartment-tenant`, `employment-iloe-private`) and `sme-rak-trading-emirati` added; the three motor personas relabelled `(insurance ga)`. EXP-02 acceptance extended with a v2.1 row (≥ 9 insurance personas across 7 lines, `status: "ga"`). New Phase 2.1 row in §11 covering full insurance line coverage + the per-line generator registry refactor. **D-08 flipped from "wire-up deferred" to "wired"**: PostHog landed end-to-end across four PRs — allowlist shim (`src/analytics.js`) + 7 call sites + lazy SDK loader (`unpkg.com/posthog-js@1`, strict EXP-21/22 options) + `POSTHOG_KEY` injection in `tools/stage-site.mjs` + Playwright route-fulfill contract (`tests/e2e/analytics.spec.mjs`). EXP-21 acceptance tightened to reference the test files. Self-approved. | 10 May 2026 |
