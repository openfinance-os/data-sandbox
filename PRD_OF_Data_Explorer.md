...
Distinct from the *user* archetypes above, the sandbox ships a curated library of synthetic UAE customer personas. **As of v2.x the library is 17 banking personas + 3 insurance preview personas.** Users load these to explore the data:
...
| **I-1** | **Motor Comprehensive — Mid** *(insurance preview)* | v2.0 | Mid-tier motor comprehensive policy, single vehicle, one named driver, no prior claims | First Phase-2 Open Insurance persona; exercises motor policy / vehicle / claims envelopes against the vendored UAE Insurance OpenAPI |
| **I-2** | **Motor Takaful Third-Party — Expat** *(insurance preview)* | v2.0 | Sharia-compliant takaful TPL policy on an older sedan, expat policyholder, Direct channel, no finance, no additional drivers | Exercises `takaful=true`, `TypeOfPolicy=ThirdPartyLiability`, and the no-finance branch of the motor generator that the comprehensive baseline doesn't reach |
| **I-3** | **Motor High-Claim Multi-Driver** *(insurance preview)* | v2.0 | Comprehensive policy on a financed family SUV, three additional named drivers, two claims in trailing 12 months (one approved), Aggregator channel | First persona with `claims_in_period > 0`, multi-driver array of length 3, and Aggregator purchase channel; tests `NoClaimsDiscountAvailable=false` and the populated-`Claims.Summary` shape |
...
| **EXP-02** | The sandbox SHALL ship a **persona library** of at least 10 named UAE customer archetypes at Phase 1 launch (per §3.3) and at least 12 by Phase 1.5. Personas are versioned and curated against publicly observable UAE-market patterns. Persona definitions contain no information sourced from any specific institution. The Phase 2 insurance preview ships at least three Open Insurance personas alongside the banking library so the motor MVP exercises the takaful flag, both `TypeOfPolicy` enum values, and a populated `Claims.Summary` shape. | At Phase 1 launch ≥ 10 banking personas published; at Phase 1.5 ≥ 12 banking personas (the v1.5 trio recorded as deferred per §3.3); at v2.0 ≥ 3 Open Insurance personas (`status: "preview"` in `dist/domains.json`). Each has a YAML manifest in `/personas/` with a `stress_coverage` field per EXP-25. Each manifest passes a "no-institution-leak" lint that flags any reference to a specific bank or LFI. | v1 / v1.5 / v2.0 |
