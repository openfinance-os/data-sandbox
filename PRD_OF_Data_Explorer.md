...
| **D-13** *(new — proposed)* | **Conversational plug-point via MCP server is permitted as an additive, read-only wrapper over `@openfinance-os/sandbox-fixtures`.** v1 ships **stdio only** (`@openfinance-os/sandbox-mcp`, `npx`-installable, runs in-process per Claude session) — preserves the §6.2 "no backend" posture in letter and spirit. The Claude marketplace listing requires a hosted **HTTP transport**, which is the *only* part of this plug-point that introduces a backend; standing it up at `mcp.openfinance-os.org/sandbox` (anonymous, CORS-open, OF-OS Commons identity per NG6) is gated on this decision. Determinism (EXP-05), spec-pin (EXP-10), watermark (EXP-19), no-PII (EXP-07), no-institution-leak (NG5), and no-mandatory-redaction (§8.3) invariants apply unchanged. | Becomes plug-point 4 alongside the four existing TPP plug-points. The hosted endpoint is the only part of the sandbox surface that *does* run a backend; the synthetic-only data corpus is what makes this acceptable — there is nothing real to protect. Stdio publication can ship under existing decisions (it is just an npm package wrapping the existing `@openfinance-os/sandbox-fixtures` runtime export) and so does not block on D-13. |
