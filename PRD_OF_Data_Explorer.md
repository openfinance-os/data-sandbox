...
| **Version** | 0.9 (draft — incorporates self-review pass: closes 4 P0 internal inconsistencies, 8 P1 ambiguities, and 8 P2 polish items; adds EXP-21/22, §6.8 URL shapes, and Hassan-validated R-EXP-04 closure signal) |
| **Date** | 28 April 2026 |
...
Distinct from the *user* archetypes above, the sandbox ships a **library of 10 synthetic UAE customer personas at Phase 1, expanding to 13 at Phase 1.5**. Users load these to explore the data:
...
| 11 | **Domestic Worker / Lower-Income Salaried** | v1.5 | AED 3–5k monthly salary, remittance-heavy outflows, simple product mix (single account, no card) | Demographic breadth — hundreds of thousands of UAE residents fit this archetype and are otherwise invisible from the persona library; tests low-balance edge cases and outbound-remittance flows |
| 12 | **High-Risk / PEP-flagged** | v1.5 | Politically Exposed Person, irregular high-value transfer patterns, additional KYC verified-claims fields populated | AML stress test — pairs directly with Aisha's JTBD-4.1 (gives her the persona to *load* when answering her job); tests `Flags` array including PEP-related markers, KYC `verifiedClaims` structure |
| 13 | **Returning Expat** | v1.5 | UAE national returning after 5 years abroad, account opened 4 months ago, age + global financial history but no UAE tenure, occasional inbound international transfers | Different from "Recent Graduate" thin-file because the customer has age and history but no UAE footprint; tests tenure-vs-file-thickness tradeoff |
...
| **EXP-06** | Persona transactions SHALL be **narratively coherent**: salaries arrive on a regular date (carrying `Flags=Payroll` per the v2.1 spec); rent direct debits hit the day after salary; merchant categories cluster (groceries, fuel, utilities, dining); FX appears only on FX-active personas; cash deposits appear only on cash-heavy personas; standing-order amounts match a transaction in the standing-order's history. Generation rules are documented in `Appendix D — Persona Generation Rulebook`. | Per-persona invariant tests: salaried personas have ≥ 1 `Flags=Payroll` transaction per month on the configured pay-day; rent direct-debit always posts within 0–3 days of salary; FX transactions only on FX-active personas; etc. Failures fail the build. | v1 |
...
- **Indexable, shareable, citable** — server-rendered key paths, OpenGraph metadata, persistent URLs. The OF-OS Commons feed surfaces it.
...
|---|---|---|---|
| **Phase 0 — Spike** | Wk 1–2 | One persona (Salaried Expat), three endpoints (accounts, balances, transactions), mandatory/optional badging only, internal preview hosted at `/commons/sandbox/preview/` (or equivalent unindexed path) with `<meta name="robots" content="noindex">`. Hardcoded LFI=Median. | Stakeholder demo: maintainer review confirms IA, field treatment, and feasibility for Phase 1 expansion. |
| **Phase 1 — V1 (Commons publication)** | Wk 3–8 | **Ten personas** (1–8 baseline + Joint Account + Senior / Retiree), all v2.1 Account Information endpoints, three LFI profiles, coverage meter, raw JSON toggle, anonymous export, share-via-URL, footer disclaimers, `/about` and citation guidance. **Published at `openfinance-os.org/commons/[slug]`.** Cross-linked from the *Credit Underwriting in the UAE* Commons article. | M1 hit (< 5 min time-to-first-insight) in usability testing with 5 users spanning anchor users (Sara) plus at least one of Hassan / Maryam / Reem / Hamid. Listed in OF-OS Commons feed. |
| **Phase 1.5 — V1.5** | Wk 9–12 | Compare-LFIs mode, Underwriting Scenario panel (with low-volume guard for the Senior persona), **persona library extended to 13** (+ Domestic Worker + PEP-flagged + Returning Expat), fixture distribution as `@openfinance-os/sandbox-fixtures` (or equivalent — naming TBD with Commons stewards). | ≥ 50 unique external monthly users; at least one external citation; AML scenario (Aisha + PEP persona) demonstrated end-to-end. |
| **Phase 2 — V2** | Q3 2026 | **Contributing guide + SME review process published first**, then community persona contributions (PR-style) opened. Open Wealth + Insurance domain extension; persona authoring tool; replay-mode (record real Al Tareq sandbox calls and replay deterministically). | Tied to Insurance Suite GA (Q2 2026 in OF roadmap); contributing guide live + ≥ 3 community-contributed personas accepted under it. |

Total v1+v1.5: **12 weeks** (Phase 1 expanded from 4 to 6 weeks in v0.9 once the realistic surface area — 10 personas × 12 endpoints × 3 LFI profiles + EXP-01 spec-driven generator + axe-core CI + Lighthouse-90 + URL-stable seeds + watermarked exports + screen-reader walk-through + EXP-27 embed mode — was costed against the prototype's ~30% coverage of Phase 1 scope). The deployment-platform decision is resolved (OF-OS Commons), the analytics surface is set (PostHog), and there is no separate contributor brand/legal/comms launch process — this is fully part of the Commons.
...
**Pre-publication checklist** (gates Phase 1 exit):
...
- PostHog analytics events instrumented and verified anonymous-only.
...
| **D-08** *(was OQ-EXP-08)* | **Analytics: PostHog** (the same surface OF-OS Commons already uses). | Anonymous events only — no PII, no fingerprinting. Captured: persona load, LFI switch, field click, endpoint navigation, raw-JSON toggle, export, share. |
| **D-09** *(was OQ-EXP-09)* | **No v2.0 → v2.1 delta view.** Out of scope across all phases. | Consistent with D-01 (v2.1 only). |
| **D-10** *(new)* | **English-only at v1; Arabic / RTL targeted for v1.5 (no later than the v1.5 release).** | UAE is bilingual; English-only at launch is a deliberate first-iteration choice, not absence-of-decision. v1.5 must ship parallel-text Arabic UI, RTL layout, Arabic-Indic numerals optional, Arabic persona narratives. The persona YAML manifest gains a `name_ar` and a `narrative_ar` field at v1.5. |
...
- **Appendix E — Companion documents**
  ...
  - `of-sandbox-prototype.html` — current single-file prototype; basis for the v1 build. Phase 0 prototype-hygiene tasks (§11) cover the residual brand-chrome sweep and the v2.1 spec corrections.
