...
if (fs.existsSync(path.join(fixtureSrc, 'manifest.json'))) {
  ...
  // Phase R4 — brand registry + placeholder SVG marks. Served at
  // /fixtures/v1/brand-registry.json + /fixtures/v1/brands/<slug>.svg.
  // Same CORS posture as bundles/ and enrichment/. The TPP-facing
  // resolution path is: bundle → enrichment.logoUrl → fetch SVG.
  ...
  if (fs.existsSync(path.join(fixtureSrc, 'brands'))) {
    cprf(path.join(fixtureSrc, 'brands'), path.join(fixtureDst, 'brands'));
  }
